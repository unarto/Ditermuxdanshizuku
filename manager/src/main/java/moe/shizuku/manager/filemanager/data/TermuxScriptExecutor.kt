...
object TermuxScriptExecutor {
    ...
    fun execute(context: Context, command: String, workingDir: String = "/"): Flow<String> = flow {
        if (!Shizuku.pingBinder()) {
            emit("Error: Shizuku is not running")
            return@flow
        }
        ...
        try {
            ...
            val process = newProcessMethod.invoke(null, arrayOf("sh", "-c", script), envArray, null) as Process
